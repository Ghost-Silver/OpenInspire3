...
struct Config {
    ...
    /**
     * @brief 逐轴二次阻力系数（N·s²/m²），0 表示无阻力
     *
     * 阻力按逐轴形式建模：`F_i = -drag_coeff * |v_i| * v_i`。
     * 真实气动阻力沿速度反方向、模长与速度平方成正比，其方向因子为
     * `v/|v|`，需要 sqrt；而 CTorch 当前没有可微的 sqrt 算子。
     * 逐轴形式不含 sqrt、全部由 abs 与乘法构成，可微性与数值稳定性都更好，
     * 且在低速段与真实阻力同阶。由此引入的速度相关力使被控对象成为非线性
     * 系统，这也是经典控制器与学习型控制器产生差异的前提。
     *
     * @note 默认 0，保证既有回归（自由落体解析解对照）不受影响。
     */
    double drag_coeff = 0.0;

    /**
     * @brief 逐轴推力限幅（牛顿），<= 0 表示不限幅
     *
     * 模拟执行器饱和。真实动力系统的推力上限由电机与桨叶决定，
     * 是控制器设计中不可忽略的约束。
     */
