...
// AABB 结构体定义
// 必须位于 VoxelMap 类之前：initObstacles 的形参 std::vector<AABB> 在类内被引用。
struct AABB {
...
struct Cell { // 单个Voxel
    ...
    bool isTargetFound() { return target_found; }
...
class VoxelMap {
  private:
    ...
    Cell& cellAt(int gx, int gy, int gz) {
        return _voxels[gz * _nx * _ny + gy * _nx + gx];
    }
  ...
  public:
    ...
    // 数据访问
    int getSearchedVoxel() const { return _searched_voxel; }
    int getSearchedTarget() const { return _searched_tar; }
    int getTotalTarget() const { return _total_target; }
};
