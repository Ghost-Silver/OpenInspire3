...
// 积分函数：从初始时间积分到终止时间
Tensor RK4Integrator::integrate(ODEFunc f, double t_start, double t_end, const Tensor& y0) {
    Tensor y = y0;
    double t = t_start;
...
// 重载积分函数：不依赖时间的 ODE
Tensor RK4Integrator::integrate(TimeIndependentODEFunc f, double t_start, double t_end, const Tensor& y0) {
    auto wrapped_func = [&f](double, const Tensor& y_in) -> Tensor {
        return f(y_in);
    };
...
Tensor make_vector_tensor(const std::vector<float>& vals) {
    Tensor t(ShapeTag{}, {static_cast<unsigned long>(vals.size())}, DType::kFloat, DeviceType::kCPU);
    float* data = const_cast<float*>(t.data<float>());
...
void test_harmonic_oscillator() {
    ...
    std::cout << "Final Result (Numerical): x = " << x_final << ", v = " << v_final << std::endl;
    std::cout << "Final Result (Analytical): x = " << analytical_x << ", v = " << analytical_v << std::endl;
    std::cout << "Absolute Error in x: " << std::abs(x_final - analytical_x) << std::endl;
    std::cout << "Absolute Error in v: " << std::abs(v_final - analytical_v) << std::endl;
}
...
void test_logistic_growth() {
    ...
    std::cout << "Final Result (Numerical): y = " << numerical_result << std::endl;
...
void test_drone_physics() {
    ...
    std::cout << "\n--- Scenario A: Free Fall ---" << std::endl;
    {
        ...
        std::cout << "Error: " << std::abs(res[2] - analytical_pd) << " m" << std::endl;
    }
    ...
    std::cout << "\n--- Scenario B: Hover ---" << std::endl;
    {
        ...
        std::cout << "Simulated Height (should stay ~0): " << res[2] << " m" << std::endl;
        std::cout << "Simulated Velocity (should stay ~0): " << res[5] << " m/s" << std::endl;
    }
}
