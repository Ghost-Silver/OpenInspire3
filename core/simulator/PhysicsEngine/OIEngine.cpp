...
Tensor drone_dynamics(const Tensor& state, const Tensor& thrust, Config config) {
    ...
    // 3. 组装导数 dx/dt
    // dx/dt = [vel, acc]
    Tensor derivative = Tensor(ShapeTag{}, {6}, DType::kFloat, DeviceType::kCPU);
...
Engine::Engine(Config config, const Tensor& state) : _config(config), _rk4(config.dt) {
    _state = state;
}

// 使用初始化列表的构造函数（推荐）
Engine::Engine(Config config) : _config(config), _rk4(config.dt) {
    // 初始化默认状态：位置和速度都为0
    _state = Tensor(ShapeTag{}, {6}, DType::kFloat, DeviceType::kCPU);
}

// 获取当前状态
Tensor Engine::getState() const {
    return _state;
}

// 设置状态
void Engine::setState(const Tensor& state) {
    _state = state;
}
...
void Engine::step(const Tensor& thrust) {
    ...
    // 使用RK4方法更新状态
    _state = _rk4.step(ode_func, 0.0, _state);
}
