...
class SixDofPidController : public SixDofController {
  public:
    ...
    /**
     * @brief 本拍**已被模型解释**的推力损失（NED 力矢量，牛顿）
     *
     * 观测器的残差是「实际加速度 − 控制器认为施加的」。若内部模型不知道入流
     * 损失，它会把这份损失当成未知扰动**再补偿一次** —— 而自适应已经补过了。
     *
     * 实测（ObserverVsAdaptiveTest 场景 A）：两者同时开启时稳态误差 0.280519，
     * 比只开自适应的 4.00543e-05 差 **7000 倍**。这不是「两个补偿叠加变好」，
     * 而是**重复补偿同一效应导致过冲**。
     *
     * 故观测器的内部模型必须包含所有已知效应：本变量记录入流补偿算出的损失，
     * 观测器在构造残差时先减掉它。这样自适应估得越准，观测器的残差越接近零，
     * 两者自然分工而不冲突。
     *
     * 由 `publish()` 每拍清零 —— 三条路径共用该出口，避免遗漏。
     */
    double _last_modeled_loss_ned[3] = {0.0, 0.0, 0.0};
